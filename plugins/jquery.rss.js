...
;(function($) {
  ...
  const RSS = function(target, url, options, callback) {
    this.target = target
  ...
  RSS.prototype.load = function(callback) {
    ...
    let apiUrl = apiHost + '?callback=?&q=' + encodeURIComponent(this.url)
  ...
  RSS.prototype.render = function() {
    ...
    this.load(function(data) {
      ...
      const html = self.generateHTMLForEntries()
      ...
      if (html.entries.length !== 0) {
        ...
        self.appendEntriesAndApplyEffects(
          $('entries', html.layout),
          html.entries
        )
      }
  ...
  RSS.prototype.appendEntriesAndApplyEffects = function(target, entries) {
    ...
    $.each(entries, function(idx, entry) {
      const $html = self.wrapContent(entry)
  ...
  RSS.prototype.generateHTMLForEntries = function() {
    ...
    $(this.entries).each(function() {
      const entry = this
      const offsetStart = self.options.offsetStart
      const offsetEnd = self.options.offsetEnd
      let evaluatedString
  ...
  RSS.prototype.executeEffectQueue = function(callback) {
    ...
    var executeEffectQueueItem = function() {
      const item = self.effectQueue.pop()
  ...
  RSS.prototype.isRelevant = function(entry, entries) {
    const tokenMap = this.getTokenMap(entry)
  ...
  RSS.prototype.getTokenMap = function(entry) {
    if (!this.feedTokens) {
      const feed = JSON.parse(JSON.stringify(this.feed))
    ...
    if (typeof moment !== 'undefined') {
      this.formattedDate = moment(new Date(entry.publishedDate)).format(
        this.options.dateFormat
      )
    } else {
      // Otherwise, if a custom formatting function is provided, use that.
      if (this.options.dateFormatFunction) {
        this.formattedDate = this.options.dateFormatFunction(
          entry.publishedDate
        )
      } else {
    ...
    return $.extend(
      {
        ...
        bodyPlain: (function(entry) {
          ...
          for (let i = 0; i < RSS.htmlTags.length; i++) {
